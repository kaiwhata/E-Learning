...
SELECT fname,lname,emailaddress,AVG(score) as average_score,COUNT(*) number_taken
 FROM useraccount LEFT JOIN result ON useraccount.id=result.userid GROUP BY useraccount.id,useraccount.fname,useraccount.lname,useraccount.emailaddress ORDER BY quiz.coursecode,AVG(score) desc;
