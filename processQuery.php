...
function getAllQuestionsFromQuiz($quizname)
{
	...
	$dbconnection = pg_connect ( $connectionString );
	$result = pg_query ( $dbconnection, "select row_to_json(row)
from(SELECT * FROM quiz INNER JOIN question ON  (quiz.name=question.quizname) LEFT OUTER JOIN possibleanswers ON (question.panswerid = possibleanswers.id) WHERE(quiz.name='".$quizname."') ;" );
	return json_encode ( pg_fetch_all ( $result ) );
}
